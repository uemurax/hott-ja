#lang morg

(require "lib/math.rkt")

(define A "A")
(define a "a")
(define x "x")
(define E "E")
(define e "e")
(define i "i")

@section[
  #:id (current-id)
  #:title @%{構造同一原理}
  @paragraph{
    一価性(と関数外延性)の帰結として、@emph{構造同一原理(structure identity principle)}をいくつか例示する。構造とは厳密に定義はしないが、なんらかの数学的構造を表すレコード型のこととする。構造同一原理は、構造の同一視型は構造を保つ同値の型と同値であることを主張する。
  }
  @paragraph{
    ある構造@(math A)と要素@(math (a . elem-of . A))に対し、同一視型@(math (abs x (a . id-type . x)))を特徴付けるには、候補@(math (E . elem-of . (A . fun-type . (universe . $ . i))))を見つけて@(math (d-pair-type (x . elem-of . A) (E . $ . x)))が可縮であることを示す。要素@(math (e . elem-of . (E . $ . a)))を自然に見つけられる場合がほとんどで、@ref["001S"]から@(math ((a . id-type . x) . equiv . (E . $ . x)))を得る。基本的な戦略は目的の型から始めて、既に可縮と分かっている型になるまでレトラクトの列を作ることである(実際には同値の列を作れる場合が多い)。すると@ref["001K"]により目的の型が可縮であることが従う。レトラクトの列を作る際には@ref["0025"]が便利で、大きなレコード型の一部に同一視型の特徴付けを知っている部分があれば、そのレコード型はより単純な型のレトラクトであることを示せる。また、レコード型の要素を並び換える関数はレトラクションであることも便利である。
  }
  (include-part "0023.rkt")
  (include-part "0028.rkt")
  (include-part "002A.rkt")
  @paragraph{
    数学でよく登場する構造といえば群や環などであるが、これらについての構造同一原理を正しく述べるには@ref["003W"]で導入する@emph{@(math "n")型}の概念を用いる。@ref["004V"]でさらなる構造同一原理を見る。
  }
]
