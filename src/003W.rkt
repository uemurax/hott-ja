...
@section[
  ...
  #:title @%{@(math n)型}
  @paragraph{
    @ref["002N"]で型は高次グルーポイドの構造を持つと説明したが、その豊富な構造をすべて把握するのは容易ではない。@(math n)次元より上の構造が自明になっているような型は@emph{@(math n)型}と呼ばれ、比較的解析が容易である。
  }
  @paragraph{
    次数@(math n)は@math{-2}から数えるのが都合がよい。
  }
  ...
  @paragraph{
    @(math trunc-level:-1)型は特別に@emph{命題}と呼ばれ、@ref["003Z"]でより詳しく調べる。@(math trunc-level:0)型は特別に@emph{集合}と呼ばれ、@ref["004B"]でより詳しく調べる。
  }
  @paragraph{
    @(math n)型の一般的な性質をいくつか見る。まず、@(math n)型はレトラクトで閉じる(@ref["0045"])。
  }
  (include-part "0046.rkt")
  (include-part "0045.rkt")
  @paragraph{
    @(math n)型はいくつかの型の構成で閉じる。
  }
