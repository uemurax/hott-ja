...
@section[
  ...
  @paragraph{
    @ref["004L"]は@ref["004J"]と比べて有益な情報が多い。その分、その定義には一つ高次の同一視(@(math is-hae:coh))を使う。
  }
  @paragraph{
  ...
  @paragraph{
    @ref["004U"]により、関数@(math f)が同値であることを示す目的では@(math (qinv . $ . f))を使って何も問題はない。@(math (qinv . $ . f))が他に挙げた正しい同値の概念と決定的に違うのは、同一でない二つの@(math (qinv . $ . f))の要素がありうる点である。後に示すが、(関数外延性の下で)@(math (is-equiv . $ . f))と@(math (is-biinv . $ . f))と@(math (is-hae . $ . f))はいずれも任意の二つの要素が同一視される(@ref["0044"]と@ref["004O"]と@ref["004R"])。@(math (qinv . $ . f))についてはこの性質は示せないどころか、一価性公理の下で@(math (qinv . $ . f))が同一でない二つの要素を持つような@(math f)を構成できることが知られている。
  }
]
