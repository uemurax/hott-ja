...
@section[
  ...
  @paragraph{
    型理論が構成的であることはプログラミング言語の観点からは重要である。構成がはっきりしない部分があると計算が進まないのでプログラムとは言えない。逆に、構成的な証明からはプログラム抽出(program extraction)ができる。例えば任意の@math{x}に対してある性質を満たす@math{y}が存在するという命題を構成的に証明できたとしよう。証明が構成的であるということは、@math{x}から具体的に@math{y}が構成されたということであるが、これは@math{x}から@math{y}を計算するプログラムであると言える。このように証明から抽出されたプログラムは定義により満たすべき性質を満たしていることが保証されている。
  }
]
